...
class GamesImport:
    ...
    def update_db(self):
        ...
        if self.games_df.size > 0:
            ...
            for index, row in games_transform_df.iterrows():
                game_log = pp.GamesImportLog(row['gameId'], game_found=1)
                game_log.insert_db()

        season_log = pp.SeasonsImportLog(team_id=self.team_id, season_id=self.season_id, games_found=games_found)
    ...
    def query_api_update_db(self):
        ...
        # clear method will drop games from a team about to be imported, removing their impending duplication
