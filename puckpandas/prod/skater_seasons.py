...
class SkaterSeasons:
    ...
    def update_db(self, season_id=0):
        if self.skater_seasons_df.size > 0:
            ...
            if season_id != 0:
                sql = """insert into puckpandas.skater_seasons (playerId, seasonId, leagueId, teamName, teamId, 
                sequence, gameType, GP, G, A, P, PM, PIM, PPG, PPP, SHG, SHP, TOIGSEC, GWG, OTG, S, SPCT, FOPCT) select 
                a.playerId, a.season as seasonId, b.leagueId, a.`teamName.default` as teamName, c.teamId, a.sequence, 
                a.gameTypeId as gameType, coalesce(a.gamesPlayed, 0) as GP, coalesce(a.goals, 0) as G, 
                coalesce(a.assists, 0) as A, coalesce(a.points, 0) as P, coalesce(a.plusMinus, 0) as PM, 
                coalesce(a.pim, 0) as PIM, coalesce(a.powerPlayGoals, 0) as PPG, coalesce(a.powerPlayPoints, 0) as PPP, 
                coalesce(a.shorthandedGoals, 0) as SHG, coalesce(a.shorthandedPoints, 0) as SHP, 
                time_to_sec(left(a.avgToi, locate(':', a.avgToi)+2))/60 as TOIGSEC, coalesce(a.gameWinningGoals, 0) 
                as GWG, coalesce(a.otGoals, 0) as OTG, coalesce(a.shots, 0) as S, coalesce(a.shootingPctg, 0) as SPCT, 
                ...
                g.teamId = t.teamId where t.teamId between 1 and 99 and g.gameType = 2 group by g.seasonId, t.teamId) 
                as c on a.season = c.seasonId and a.`teamName.default` = c.fullName where 
                ...
                t.fullName, count(gameId) as games from puckpandas.game_results as g join puckpandas.teams as t on 
                g.teamId = t.teamId where t.teamId between 1 and 99 and g.gameType = 2 group by g.seasonId, t.teamId) 
                as c on a.season = c.seasonId and a.`teamName.default` = c.fullName"""
